...
export async function getScoreLimits(): Promise<Record<string, number>> {
  ...
  try {
    ...
    // score_limits.machine is a canon key, as is every machine name the callers
    // pass in, so one case-folded entry per row matches exactly. This used to
    // fan out over spelling variations, which still missed limits stored under
    // a display name (Star Trek (Stern) and Black Knight SOR matched nothing).
